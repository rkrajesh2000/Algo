...
class Test{  
	...
	public static void main(String args[]){  	
		...
		System.out.println("largestRectangleInHistogram() : " + largestRectangleInHistogram(new int[] {2,1,5,6,2,3}));		
	 }
...
//	        }    
    ...
    private static int HouseRobber(int[] nums){
	  ...
	  * find the area of largest rectangle in the histogram. A histogram where width of each bar is 1, given height = [2,1,5,6,2,3].
	 ...
	 private static int largestRectangleInHistogram(int[] heights){
		 ...
		 while(i < heights.length ){
			 ...
			 if(stack.isEmpty() || heights[i] >= heights[stack.peek()] ){
				 ...
				 int top = stack.pop();				 
				 maxArea = Math.max(maxArea, heights[top] * (stack.empty() ? i : i - 1 - stack.peek()));
			 }
		}
